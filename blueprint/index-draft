...
**NOTE:** `Process Automation Triggers` is currently in beta. To gain access to this feature you should contact your CSM/TAM or reach out to Richard Schott at richard.schott@genesys.com. Please make sure you have required access before proceeding with the blueprint.

This Genesys Cloud Developer Blueprint demonstrates how to save participant attributes to a third-party database through the use of event triggers. The infrastucture is deployed using using Terraform, CX as Code, and Archy. The third-paty databse used in this blue print is Amazon's s3 bucket. In the proess of deploying the infrastructure, Terraform will deploy all the required AWS resources, genesys cloud resources, architect flows, and finally, run a python script to create a process trigger which is very imperative in this system. The process trigger enables you to define events or conditons that will enable your workflow to be executed in response to defined events(which are disconnected calls in this case).

![Event Orchestration flowcart](images/flowchart.png "Deploy a Process Automation Triggers setup")
...
If you have an existing DID pool range you want to use, open the `telephony.tf` file and comment out the `genesyscloud_telephony_providers_edges_did_pool` resource. Proceed to comment out the `depends_on` property in the `genesyscloud_architect_ivr` resource as well and save your changes.
